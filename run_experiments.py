...
hyperparameter_configuration= {
          ...
          'parameters': {
              'Task_Weighting_strategy': {
                  'values': [AlgType.SLGrad.value, AlgType.PCGrad.value] #supported weighting algorithms see datasets.py
              },
              'Dataset': {
                  'values': [DataName.CIFAR10.value] #supported datanames see datasets.py
              },
              'Number_of_Tasks': {
                  'values': [2]#[numTask]
              },
              ...
              'Epochs': {
                  'values': [3] #np.arange(minLimEpoch, maxLimEpoch, 1).tolist()
              },
              'Batch_Size': {
                  'values': [16]#[256] #np.arange(minLimBatchsize, maxLimBatchsize, 1).tolist()
              },
              'val_Batch_size': {
                  'values': [32]
              },
              ...
              'Dim_Task_Layers': {
                  'values': [84] #np.arange(minLimDimTaskLayer, maxLimDimTaskLayer, 1).tolist()
              },
              'Optimizer': {
                  'values': ["sgd"] #['adam', 'sgd']#
              },
